...
class DirIndex(ItemData):
    ...
    * Key offset: Index in the directory.
    ...
    def __init__(self, header, data):
        if header == None:
            self.location = Key(data[0],data[1],data[2])
            self.key = Key(data[9],data[10],data[11])
            self._setattr_from_key(objectid_attr='keyobjectid', type_attr='keytype', offset_attr='index')
            self.objectid = ULL(int(data[0]))
            self.index = ULL(int(data[4]))
            self.type = U8(int(data[5]))
            self.name_len = len(data[7])
            self.name = data[7]
        else:
            super().__init__(header)
...
class FileExtentItem(ItemData):
    ...
    * Key offset: Logical offset in the file where the referenced data appears.
    ...
    def __init__(self, header, data):
        if header == None:
            self.logical_offset = ULL(int(data[0]))
            self.generation = ULL(int(data[1]))
            self.ram_bytes = ULL(int(data[2]))
            self.compression = U8(int(data[3]))
            self.type = U8(int(data[5]))
            if self.type != FILE_EXTENT_INLINE:            
                self.disk_bytenr = ULL(int(data[7]))
                self.disk_num_bytes = ULL(int(data[8]))
                self.offset = ULL(int(data[9]))
                self.num_bytes = ULL(int(data[10]))
                self._inline_encoded_nbytes = None
                self.logical_bytes = self.num_bytes
            else:
                self.inline_encoded_nbytes = ULL(int(data[11]))
                self.disk_bytenr = None
                self.disk_num_bytes = None 
                self.offset = None
                self.num_bytes = None
                self.logical_bytes = self.ram_bytes
            self.key = Key(data[12],data[13],data[14])
            self._setattr_from_key(objectid_attr='keyobjectid', offset_attr='logical_offset')
        else:
            super().__init__(header)
